  ...
  <div>
    ...
    <div>
      ...
      <div v-else-if="!testEnded">
        <div @click="isTestStartedFunc" class="btn btn-primary mx-3">
          Start Test
        </div>
      </div>
      <div class="col-md-4">
...
export default {
  ...
  created() {
    ...
    this.questions = this.questions.map((question) => {
      if (question.qType === "single") question.answer = "";
      else if (question.qType === "multiple") question.answer = [];
    ...
    handleDataAvailable(event) {
      ...
      if (event.data.size > 0) {
        if (this.socket.connected) {
          ...
          alert(
            "Socket disconnnected! Some error occured. Please retry the upload"
          );
        ...
        .then((result) => {
          const facePredict = async () => {
            ...
            if (!predictions.length)
              return {
                lookedAway: true,
                numberOfPeople: predictions.length,
                confidence: 0,
              };
            const leftCheek = predictions[0].annotations.leftCheek[0];
            const midwayBetweenEyes =
              predictions[0].annotations.midwayBetweenEyes[0];
            const noseTip = predictions[0].annotations.noseTip[0];
            const leftIrisError = this.calculateError(
              predictions[0].annotations.leftEyeIris
            );
            const rightIrisError = this.calculateError(
              predictions[0].annotations.rightEyeIris
            );
            ...
            if (leftCheek[2] >= 18 || leftCheek[2] <= -18) {
              ...
              midwayBetweenEyes[2] >= 5 ||
              midwayBetweenEyes[2] <= -20
            ) {
              lookedAway = true;
            ...
            return {
              lookedAway,
              numberOfPeople: predictions.length,
              confidence: predictions[0].faceInViewConfidence,
            };
          };
          const mobilePredict = async () => {
            const predictions = await result[1].detect(
              document.querySelector("video")
            );
            return {
              mobile: predictions.some(
                (prediction) => prediction.class === "cell phone"
              ),
            };
          };
          const predict = () => {
            return Promise.all([facePredict(), mobilePredict()]).then(
              (result) => {
                return {
                  ...result[0],
                  ...result[1],
